...
export const ResultsView: React.FC<Props> = ({ result, formData, language }) => {
  ...
  // 1. CRITICAL SAFETY CHECK: If result is null/undefined, render nothing to prevent crash
  if (!result) return null;

  // 2. DATA SANITIZATION: Robustly handle missing or malformed data
  // Ensure competitors is an array, and filter out any non-object items (e.g. null strings)
  const competitors = Array.isArray(result.competitors) 
    ? result.competitors.filter(c => c && typeof c === 'object') 
    : [];
  ...
  // 3. SAFE FORMATTING: Format data for the text report
  const competitorReportData = competitors.map((c, i) => 
    `${i+1}. ${c.name || 'Unknown Brand'}\n   Features: ${c.features || 'N/A'}\n   Price: ${c.price || 'N/A'}`
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 relative group">
        ...
        <div className="absolute top-6 right-6">
             <AddToReportButton 
                ...
                data={result.marketSummary || "No summary"} 
                language={language} 
            />
        </div>
      </div>
            ...
            {competitors.length > 0 ? competitors.map((comp, idx) => (
              <div key={idx} className="p-4 border border-slate-100 rounded-lg hover:bg-slate-50 transition-colors">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-medium text-slate-900">{comp.name || "Unknown Brand"}</h4>
                    <p className="text-sm text-slate-500 mt-1 line-clamp-2">{comp.features || "No details"}</p>
                  </div>
              ...
              <div className="text-center py-8 text-slate-400 text-sm italic">
                No competitor data returned by AI.
              </div>
            )}
          </div>
        </div>
